...
data1 <- data[1:128,] # en el csv los datos están ordenados por años, por eso el año 1 va desde la fila 1 a la 128. Fila 1 porque no se cuenta el encabezado
...
# abundancia total de artropodos
abund.dh1 <- dh1$abund 
abund.ch1 <- ch1$abund 
abund.lg1 <- lg1$abund 
abund.av1 <- av1$abund 
...
#manejo = Orgánico(1)-Convencional(0) (covariable)
...
# Estacion = Verano(1)-Primavera(0) (covariable)
...
####################################################################################################
##  AÑO 2 - lectura de los datos del año 2
data2 <- data[129:256,] # 129 es la última fila del csv, como no se cuenta el encabezado se pone 156

dh2 <- subset(data2, data2$farm == 'DH')
av2 <- subset(data2, data2$farm == 'AV')
lg2 <- subset(data2, data2$farm == 'LG')
ch2 <- subset(data2, data2$farm == 'CH')
...
#manejo = Orgánico(2)-Convencional(0) (covariable)
...
########## unir las matrices de ambos años
...
out <- jags(win.data, inits, params, "M4-GLMMPoisson.txt", n.chains = nc, n.thin = nt,
...
### GRAFICAR Y EXPORTAR EL GRAFICO
tiff(file = "M4-GLMMPoisson_COMPLETO.tiff",                #Guardados como Tiff
